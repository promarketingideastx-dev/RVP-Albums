    ...
    if (isDefault) {
        ...
        <div className="flex flex-col h-full bg-white dark:bg-neutral-950 p-4">
          ...
          {assets.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-3 overflow-y-auto pr-1">
              {assets.map((asset) => (
                <div key={asset.id} className="aspect-square bg-neutral-100 dark:bg-neutral-900 rounded-md border border-neutral-200 dark:border-neutral-800 relative overflow-hidden cursor-grab active:cursor-grabbing hover:border-blue-400 transition-all hover:shadow-sm" draggable onDragStart={(e) => handleDragStartDefault(e, asset, activeCategory)} title={asset.name}>
      ...
      return (
        <div className="flex flex-col h-full bg-white dark:bg-neutral-950 p-4">
          ...
          <div className="grid grid-cols-2 gap-3 overflow-y-auto pr-1">
            {/* UPLOAD TILE */}
            <button onClick={() => fileInputRef.current?.click()} className="aspect-square bg-blue-50 dark:bg-blue-900/10 rounded-md border border-dashed border-blue-300 dark:border-blue-800 flex flex-col items-center justify-center text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900/20 hover:border-blue-400 transition-all cursor-pointer">
            ...
            {decs.map((dec) => (
              <div key={dec.id} className="aspect-square bg-neutral-100 dark:bg-neutral-900 rounded-md border border-neutral-200 dark:border-neutral-800 relative overflow-hidden cursor-grab active:cursor-grabbing hover:border-blue-400 transition-all hover:shadow-sm group" draggable onDragStart={(e) => handleDragStartUser(e, dec)}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={dec.preview} alt="user decoration" className="absolute inset-0 w-full h-full object-contain pointer-events-none p-2" />
                <button onClick={(e) => handleDeleteDecoration(e, dec.id)} className="absolute top-1 right-1 bg-white dark:bg-neutral-800 rounded-full p-1 opacity-0 group-hover:opacity-100 shadow-sm border border-neutral-200 dark:border-neutral-700 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-500 transition-all">
